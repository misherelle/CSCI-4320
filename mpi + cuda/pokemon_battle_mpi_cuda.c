/*
  CSCI-4320 Group Project - Pokemon Battle Territory MPI + CUDA Layer
*/
...
int main(int argc, char **argv)
{
    ...
    for (int step = 0; step < steps; step++) {
        ...
        if (req_count > 0) {
            rc = MPI_Waitall(req_count, reqs, MPI_STATUSES_IGNORE);
            mpi_fail(rc, "MPI_Waitall halo exchange", rank);
        }

        comm_seconds += ticks_to_seconds(aimos_clock_read() - comm_start);
        ticks compute_start = aimos_clock_read();
        pokemon_cuda_update_local_rows(&current, &next, local_rows, rank, nprocs);
        compute_seconds += ticks_to_seconds(aimos_clock_read() - compute_start);
    ...
    if (rank == 0) {
        ...
        printf("Pokemon Battle Territory MPI + CUDA row-decomposition run\n");
